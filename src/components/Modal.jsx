...
import { useAddNewTaskMutation } from "../redux/task/taskApi";
...
export const Modal = () => {
  ...
  const [addNewTask, { isError, data: newTask }] = useAddNewTaskMutation();
  // create task
  ...
  return (
    <>
      <button
        ...
        className="block text-white  bg-violet-500 hover:bg-violet-700 transition font-medium rounded-lg text-sm px-5 py-2.5 text-center "
        type="button"
      >
        + Task
      </button>
      ...
      <div
        ...
        className={
          !toggle
            ? "fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto  h-[calc(100%-1rem)] max-h-full"
